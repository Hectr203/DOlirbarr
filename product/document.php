	...
	*/
	...
	/** inicio
	 * 
	 * se sube ala base de datos el archivo de la iamgen del producto
	 * 
	 */
	 ...
	 if ($action == 'upload_image') {
		...
		if (!empty($_FILES['product_image']['name'])) {
			...
			if (dol_move_uploaded_file($filetmp, $upload_dir.'/'.$filename, 0)) {
				// Insertar la ruta de la imagen en la base de datos
				$sql = "UPDATE llx_product SET Img_Products = '".$db->escape($filename)."' WHERE rowid = ".$object->id;
				$resql = $db->query($sql);
				if ($resql) {
					setEventMessages($langs->trans("Imagen guardada correctamente"), null, 'mesgs');
				} else {
					setEventMessages($langs->trans("Error al cargar el archivo"), null, 'errors');
				}
			} else {
				setEventMessages($langs->trans("Error al mover archivo"), null, 'errors');
			}
		} else {
			setEventMessages($langs->trans("No se ha subido ningún archivo"), null, 'errors');
		}
	}
...
/**
 * 
 *inicio del formulario (elemto para subir solo la iamgen del producto ) 
 * 
 * 
 */
		// Mostrar formulario para subir imagen del producto
...
// Campo para seleccionar la imagen del producto
print '<div class="form-group">';
print '<label for="product_image">'.$langs->trans("Imagen del producto").'</label>';
print '<input type="file" name="product_image" id="product_image" class="form-control">';
...
//fin del formulario (elemto para subir solo la iamgen del producto ) 
